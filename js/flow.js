...
export const TOURNAMENT_FLOW_VERSION = "mobbr-tournament-flow-1.0.0";
...
function teamRosterRow(team, index, playerTeamId) {
  ...
  return `
    <article class="team-roster-row team-roster-row--pending">
      <span class="team-roster-row__number">${String(index + 1).padStart(2, "0")}</span>
      <div class="team-roster-row__placeholder" aria-hidden="true">?</div>
      <div>
        <strong>CPU SLOT ${String(team.slotNumber).padStart(2, "0")}</strong>
        <small>${escapeHtml(team.source)} / 正式データは生成10で接続</small>
      </div>
      <em>PENDING</em>
    </article>
  `;
      ...
      <div class="tournament-scroll-area">
        ...
        <section class="team-roster-panel">
          <header>
            ...
            <p>
              National・Worldの正式グループ構成は、CPUチームデータ接続時に確定します。
            </p>
          </header>
    ...
    >
      ...
      <section class="exploration-handoff-card">
        ...
        <div class="generation-boundary">
          ...
          <p>
            探索候補・施設・CPUチーム・ROUND・戦闘は次工程以降で接続します。
            未確定報酬は一切付与されていません。
          </p>
        </div>
      </section>
...
function matchStartTemplate(runtime) {
  return provisionalPhaseTemplate(runtime, {
    ...
    description:
      "MATCH限定状態を初期化しました。戦闘・探索の本処理は後続工程で接続します。",
...
function encounterPreviewTemplate(runtime) {
  ...
  return provisionalPhaseTemplate(runtime, {
    ...
    title: "対戦相手データ待機中",
    description:
      "プレイヤー側は正式参加データです。CPU対戦相手は生成10のCPUマスター接続後に確定します。",
    commentary: "対戦相手のCPUデータを待ちながら、作戦準備へ移ります。",
    ...
    content: `
      <div class="encounter-preview-grid">
        ...
        <section class="encounter-team encounter-team--pending">
          <span>CPU TEAM</span>
          <h2>ROSTER PENDING</h2>
          <div class="pending-opponent-mark">?</div>
          <p>${escapeHtml(runtime.entryData.tournament.cpuPoolId)}</p>
        </section>
      </div>
    `,
  });
...
function battleDiagnosticTemplate(runtime) {
  return provisionalPhaseTemplate(runtime, {
    eyebrow: "BATTLE DIAGNOSTIC",
    title: "戦闘計算は未接続",
    description:
      "生成11で通常攻撃・弾薬・ダウン・確キル・回復・復活・3スキル・作戦・特殊能力を接続します。今回はHPと戦績を変更しません。",
    commentary: "診断用の仮戦闘です。ダメージ・キル・報酬は発生しません。",
    primaryAction: "battle-diagnostic-next",
    primaryLabel: "仮OUTCOME",
    content: `
      <div class="diagnostic-zero-grid">
        <span>DAMAGE <strong>0</strong></span>
        <span>KILL <strong>0</strong></span>
        <span>DOWN <strong>0</strong></span>
        <span>ITEM USE <strong>0</strong></span>
      </div>
    `,
  });
}
...
function battleOutcomeTemplate(runtime) {
  return provisionalPhaseTemplate(runtime, {
    ...
    title: "NO RESULT CALCULATED",
    description:
      "戦闘モジュール未接続のため、両チームの勝敗・HP・戦績は変更していません。",
    commentary: "仮ROUND RESULTへ安全に進みます。",
...
function roundResultTemplate(runtime) {
  const members = playerMembers(runtime);
  return provisionalPhaseTemplate(runtime, {
    eyebrow: "PROVISIONAL ROUND RESULT",
    title: `MATCH ${runtime.match} / ROUND ${runtime.round}`,
    description:
      "固定進行表の仮ROUND RESULTです。戦闘計算前のため全数値は0、HPは参加時のままです。",
    commentary: "ROUND診断結果を確認します。正式順位はまだ計算しません。",
    primaryAction: "round-result-next",
    primaryLabel: "仮MATCH判定",
    content: `
      <div class="provisional-result-list">
        ${members.map((member) => `
          <article>
            <img src="${escapeAttribute(member.image)}" alt="">
            <div>
...
function matchResultTemplate(runtime) {
  return provisionalPhaseTemplate(runtime, {
    ...
    commentary: "生成9の大会ランタイム確認は完了です。安全に中断保存できます。",
      ...
      try {
        ...
        if (action === "round-intro-next") {
          runtimeManager.transition("ENCOUNTER_PREVIEW", {
            reason: "provisional_round_intro_completed",
        ...
        if (action === "battle-diagnostic-next") {
          runtimeManager.transition("BATTLE_OUTCOME", {
            reason: "provisional_battle_diagnostic_completed",
        ...
        if (action === "battle-outcome-next") {
          runtimeManager.update("provisional_round_record_created", (draft) => {
            if (!draft.roundTotals.some(
              (record) => record.match === draft.match && record.round === draft.round,
            )) {
              draft.roundTotals.push({
                match: draft.match,
                round: draft.round,
                provisional: true,
                ...
                kills: 0,
                assists: 0,
                activeTeams: draft.activeTeamIds.length,
              });
            }
          });
          runtimeManager.transition("ROUND_RESULT", {
            reason: "provisional_round_result_ready",
            patch: { pendingVisualId: "round-result" },
          });
          runtimeManager.checkpoint("provisional_round_result");
          render();
          return;
        }
