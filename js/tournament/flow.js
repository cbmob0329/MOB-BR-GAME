 ...
 *
 * Generation 9 scope: automatic entry loading, opening scenes, team intro,
 * deployment, safe suspension/resume, runtime checkpoints, and errors.
 */
...
export const TOURNAMENT_FLOW_VERSION = "mobbr-tournament-flow-1.2.0";
...
function initialExplorationTemplate(runtime) {
  const carryCount = runtime.inventory.slots.filter(Boolean).length;
  return `
    <main
      class="tournament-screen tournament-screen--exploration-handoff"
      style="--map-background:url('${escapeAttribute(runtime.map.image)}')"
    >
      ${topStatusTemplate(runtime)}
      <section class="exploration-handoff-card">
        <span>INITIAL EXPLORATION</span>
        <h1>大会ランタイム準備完了</h1>
        <p>
          参加データ・選手・武器・全50作戦・バッグ・コーチ・特殊能力を
          大会用ランタイムへ複製しました。
        </p>
        <div class="runtime-ready-grid">
          <article><span>TEAMS</span><strong>${runtime.teams.length}</strong></article>
          <article><span>PLAYERS</span><strong>${Object.keys(runtime.memberRuntime).length}</strong></article>
          <article><span>STRATEGIES</span><strong>${Object.keys(runtime.strategyRuntime).length}</strong></article>
          <article><span>CARRY</span><strong>${carryCount} / ${runtime.inventory.capacity}</strong></article>
        </div>
        <div class="generation-boundary">
          <strong>GENERATION 9 COMPLETE POINT</strong>
          <p>
            CPUチームは正式データへ接続済みです。探索・施設・戦闘計算は後続工程で接続します。
            未確定報酬は一切付与されていません。
          </p>
        </div>
      </section>
      <div class="tournament-bottom-area">
        ...
        <div class="tournament-actions">
          ...
          <button
            type="button"
            class="tournament-button tournament-button--primary"
            data-action="initial-exploration-next"
          >
            仮MATCH START
          </button>
        </div>
...
function matchStartTemplate(runtime) {
  return provisionalPhaseTemplate(runtime, {
    ...
    description:
      "正式CPUロスターを含むMATCH限定状態を初期化しました。戦闘・探索の本処理は後続工程で接続します。",
...
function strategyCheckTemplate(runtime) {
  const defaultStrategy = runtime.strategyRuntime["D-01"];
  const usableCount = Object.values(runtime.strategyRuntime).filter(
    (strategy) => strategy.unlimited || strategy.tournamentRemaining > 0,
  ).length;
  return provisionalPhaseTemplate(runtime, {
    eyebrow: "STRATEGY SELECT",
    title: defaultStrategy?.name ?? "D-01",
    description:
      "生成9ではD-01を安全なフォールバックとして仮確定します。正式な全50作戦選択UIと消費処理は生成13で接続します。",
    commentary: `${defaultStrategy?.name ?? "バランスを大事に"}で仮戦闘診断へ入ります。`,
    primaryAction: "strategy-next",
    primaryLabel: "BATTLE CHECK",
    content: `
      <div class="strategy-diagnostic">
        <span>LOADED STRATEGIES</span>
        <strong>${Object.keys(runtime.strategyRuntime).length}</strong>
        <small>使用可能 ${usableCount} / 仮選択 D-01 / 未消費</small>
      </div>
    `,
  });
}
...
function battleCountdownTemplate(runtime) {
  return provisionalPhaseTemplate(runtime, {
    ...
    title: "3 · 2 · 1",
    description:
      "戦闘ランタイム生成直前の安全な復元地点です。",
    commentary: "戦闘診断を開始します！",
    primaryAction: "battle-countdown-now",
    primaryLabel: "START",
    content: `<div class="countdown-pulse">BATTLE</div>`,
  });
}

function battleDiagnosticTemplate(runtime) {
  const opponent = getCurrentCpuOpponent(runtime);
  const playerTeam = runtime.teams.find(
    (team) => team.teamId === runtime.playerTeamId,
  );
  return provisionalPhaseTemplate(runtime, {
    eyebrow: "AUTO BATTLE",
    title: `${playerTeam?.teamName ?? "PLAYER"} VS ${opponent?.teamName ?? "CPU"}`,
    description:
      "3対3・10秒・0.1秒tickの決定的自動戦闘を実行します。戦闘開始後の攻撃操作はありません。",
    commentary: "通常攻撃・弾薬・リロード・スキル・ダウン・確キルを計算します！",
    primaryAction: "battle-run-now",
    primaryLabel: "AUTO BATTLE実行",
    content: `
      <div class="diagnostic-zero-grid battle-rule-grid">
        <span>FORMAT <strong>3 VS 3</strong></span>
        <span>TIME <strong>10.0s</strong></span>
        <span>TICK <strong>0.1s</strong></span>
        <span>AMMO <strong>8</strong></span>
      </div>
    `,
  });
}
...
function battleOutcomeTemplate(runtime) {
  ...
  if (!result) {
    return provisionalPhaseTemplate(runtime, {
      eyebrow: "BATTLE OUTCOME",
      title: "RESULT NOT FOUND",
      description: "戦闘結果が生成されていません。",
      commentary: "戦闘結果を確認できません。",
      primaryAction: "battle-run-now",
      primaryLabel: "戦闘を再実行",
    });
  }
  const winnerName = result.draw
    ? "DRAW"
    : runtime.teams.find(
        (team) => team.teamId === result.winnerTeamId,
      )?.teamName ?? result.winnerTeamId;
  const playerSummary = result.teamSummaries[runtime.playerTeamId];
  const opponentId =
    result.winnerTeamId === runtime.playerTeamId
      ? result.loserTeamId
      : result.winnerTeamId === null
        ? runtime.currentOpponentId
        : result.winnerTeamId;
  const opponentSummary = result.teamSummaries[opponentId];

  return provisionalPhaseTemplate(runtime, {
    eyebrow: result.draw ? "DRAW" : "BATTLE OUTCOME",
    title: result.draw ? "DRAW" : `${escapeHtml(winnerName)} WIN`,
    description:
      `${result.elapsedSeconds.toFixed(1)}秒 / ${escapeHtml(result.endReason)} / 判定 ${escapeHtml(result.tieBreaker ?? "なし")}`,
    commentary:
      result.winnerTeamId === runtime.playerTeamId
        ? "プレイヤーチームが戦闘を制しました！"
        : result.draw
          ? "両チーム譲らずドローです！"
          : "対戦CPUチームが戦闘を制しました。",
    primaryAction: "battle-outcome-next",
    primaryLabel: "ROUND RESULT",
    content: `
      <div class="battle-outcome-grid">
        <article class="${result.winnerTeamId === runtime.playerTeamId ? "is-winner" : ""}">
          <span>PLAYER</span>
          <strong>${escapeHtml(runtime.entryData.playerTeam.teamName)}</strong>
          <small>
            DMG ${formatNumber(playerSummary.damage)} /
            DOWN ${playerSummary.downsGiven} /
            KP ${playerSummary.kp}
          </small>
        </article>
        <article class="${result.winnerTeamId === opponentId ? "is-winner" : ""}">
          <span>CPU</span>
          <strong>${escapeHtml(runtime.teams.find((team) => team.teamId === opponentId)?.teamName ?? opponentId)}</strong>
          <small>
            DMG ${formatNumber(opponentSummary?.damage ?? 0)} /
            DOWN ${opponentSummary?.downsGiven ?? 0} /
            KP ${opponentSummary?.kp ?? 0}
          </small>
        </article>
      </div>
      <p class="result-contract-note">
        EVENTS ${formatNumber(result.eventCount)} /
        TICKS ${formatNumber(result.tickCount)}
      </p>
    `,
  });
}
  ...
  function render() {
    ...
    switch (runtime.phase) {
      ...
      case "INITIAL_EXPLORATION":
        root.innerHTML = initialExplorationTemplate(runtime);
        break;
      case "MATCH_START":
      ...
      case "STRATEGY_SELECT":
        root.innerHTML = strategyCheckTemplate(runtime);
        break;
      case "BATTLE_COUNTDOWN":
        ...
        scheduleAction(() => {
          try {
            runtimeManager.transition("BATTLE", {
              reason: "provisional_countdown_completed",
            });
            render();
          } catch (error) {
            handleRuntimeError(error);
        ...
        break;
      case "BATTLE":
        root.innerHTML = battleDiagnosticTemplate(runtime);
        break;
      case "BATTLE_OUTCOME":
      ...
      case "ROUND_ADVANCE":
        root.innerHTML = provisionalPhaseTemplate(runtime, {
          ...
          title: "仮MATCH終了判定",
          description: "診断用1ROUNDを確定し、MATCH CHAMPION判定へ移ります。",
          commentary: "仮MATCH CHAMPION判定へ進みます。",
          primaryAction: "round-advance-next",
          primaryLabel: "CHAMPION CHECK",
        });
        break;
      case "MATCH_CHAMPION":
  ...
  root.addEventListener("click", (event) => {
    ...
    withUiLock(() => {
      try {
        ...
        if (action === "initial-exploration-next") {
          runtimeManager.transition("MATCH_START", {
            reason: "provisional_initial_exploration_completed",
            patch: { match: 1, round: 0, pendingVisualId: "match-start" },
          });
          runtimeManager.checkpoint("provisional_match_start");
          render();
          return;
        }
        ...
        if (action === "round-intro-next") {
          runtimeManager.update("cpu_opponent_locked", (draft) => {
            ...
            const opponentId =
              cpuTeamIds[(draft.round - 1) % cpuTeamIds.length];
            draft.currentOpponentId = opponentId;
            draft.lockedOpponentId = opponentId;
            draft.currentPairs = [
              [draft.playerTeamId, opponentId],
            ];
          });
        ...
        if (action === "encounter-next") {
          runtimeManager.transition("STRATEGY_SELECT", {
            reason: "provisional_encounter_confirmed",
            patch: { pendingVisualId: "strategy-select" },
          });
          render();
          return;
        }
        if (action === "strategy-next") {
          runtimeManager.update("provisional_strategy_selected", (draft) => {
            draft.teamRuntime[draft.playerTeamId].currentStrategyId = "D-01";
            draft.teamRuntime[draft.playerTeamId].strategyConsumed = false;
          });
          runtimeManager.transition("BATTLE_COUNTDOWN", {
            reason: "provisional_strategy_ready",
            patch: { pendingVisualId: "battle-countdown" },
          });
          runtimeManager.checkpoint("provisional_battle_countdown");
          render();
          return;
        }
        if (action === "battle-countdown-now") {
          runtimeManager.transition("BATTLE", {
            reason: "provisional_countdown_skipped",
          });
          render();
          return;
        }
        if (action === "battle-run-now") {
          const transaction = runtimeManager.update(
            "deterministic_battle_completed",
            (draft) => executeCurrentBattleToDraft(draft),
          );
          runtimeManager.transition("BATTLE_OUTCOME", {
            reason: "battle_engine_completed",
            patch: { pendingVisualId: "battle-outcome" },
          });
          showToast(
            `戦闘完了：${transaction.result.result.elapsedSeconds.toFixed(1)}秒`,
          );
          render();
          return;
        }
        ...
        if (action === "round-advance-next") {
          runtimeManager.transition("MATCH_CHAMPION", {
            reason: "provisional_match_end_reached",
